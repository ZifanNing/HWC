...
def compute_prd_from_embedding(eval_data, ref_data, num_clusters=20, num_angles=1001, num_runs=10,enforce_balance=True):
    ...
    if enforce_balance and len(eval_data) != len(ref_data):
        raise ValueError('The number of points in eval_data %d is not equal to the number of points in ref_data %d. To disable this exception, set enforce_balance to False (not recommended).' % (len(eval_data), len(ref_data)))
