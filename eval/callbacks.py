...
def _sample_cb(log, config, visdom=None, pdf=None, test_datasets=None, sample_size=64, iters_per_task=None):
    ...
    def sample_cb(generator, batch, task=1, epoch=None, allowed_classes=None, **kwargs):
        ...
        if iteration % log == 0:
            statement = ' after {} iters{}{}'.format(batch, '' if epoch is None else ' (epoch {})'.format(epoch), '' if iters_per_task is None else ' in task {}'.format(task))

            # Evaluate reconstruction-ability of model on [test_dataset]
            if test_datasets is not None:
                # Reconstruct samples from current task
                evaluate.show_reconstruction(generator, test_datasets[task - 1], config, size=int(sample_size / 2), pdf=pdf, visdom=visdom, epoch=epoch, task=task)

            # Generate samples
            evaluate.show_samples(generator, config, pdf=pdf, visdom=visdom, size=sample_size, allowed_classes=allowed_classes, allowed_domains=list(range(task)), title='Generated images{}'.format(statement))
...
def _eval_cb(log, test_datasets, visdom=None, precision_dict=None, iters_per_task=None, test_size=None, classes_per_task=None, scenario='none'):
    ...
    def eval_cb(classifier, batch, task=1, **kwargs):
        ...
        if iteration % log == 0:
            evaluate.precision(classifier, test_datasets, task, iteration, classes_per_task=classes_per_task, scenario=scenario, precision_dict=precision_dict, test_size=test_size, visdom=visdom)
...
def _latent_space_cb(log, datasets, visdom=None, pdf=None, sample_size=400, iters_per_task=None):
    ...
    def latent_space_cb(model, batch, task=1, **kwargs):
        ...
        if iteration % log == 0:
            ...
            evaluate.visualize_latent_space(model, X.to(model._device()), y=y.to(model._device()), visdom=visdom,pdf=pdf)
...
def _solver_loss_cb(log, visdom, model=None, tasks=None, iters_per_task=None, epochs=None, rnt=None, replay=False, progress_bar=True):
    ...
    def cb(bar, iter, loss_dict, task=1, epoch=None):
        ...
        if progress_bar and bar is not None:
            ...
            bar.set_description(' <MAIN MODEL> |{t_stm}{e_stm} training loss: {loss:.3} | training precision: {prec:.3} |'.format(t_stm=task_stm, e_stm=epoch_stm, loss=loss_dict['loss_total'], prec=loss_dict['precision']))
            bar.update(1)
        ##-----------------------------------------------------------------------------##
        ...
        if (iteration % log == 0) and (visdom is not None):
            ...
            visual.visdom.visualize_scalars(scalars=plot_data, names=names, iteration=iteration, title='CLASSIFIER: loss ({})'.format(visdom['graph']), env=visdom['env'], ylabel='training loss')

    # Return the callback-function.
    return cb

def _VAE_loss_cb(log, visdom, model, tasks=None, iters_per_task=None, epochs=None, rnt=None, replay=False, progress_bar=True):
    ...
    def cb(bar, iter, loss_dict, task=1, epoch=None):
        ...
        if progress_bar and bar is not None:
            task_stm = '' if (tasks is None) else ' Task: {}/{} |'.format(task, tasks)
            epoch_stm = '' if ((epochs is None) or (epoch is None)) else ' Epoch: {}/{} |'.format(epoch, epochs)
            bar.set_description(' <GENERATOR>  |{t_stm}{e_stm} training loss: {loss:.3} | training precision: {prec:.3} |'.format(t_stm=task_stm, e_stm=epoch_stm, loss=loss_dict['loss_total'], prec=loss_dict['precision']))
            bar.update(1)
        ##-----------------------------------------------------------------------------##
        ...
        if (iteration % log == 0) and (visdom is not None):
            ...
            visual.visdom.visualize_scalars(scalars=plot_data, names=names, iteration=iteration, title='VAE: loss ({})'.format(visdom['graph']), env=visdom['env'], ylabel='training loss')
