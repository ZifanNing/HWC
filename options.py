...
def add_eval_options(parser, single_task=False, generative=False, compare_code='none', only_MNIST=True, **kwargs):
    ...
    if compare_code == 'none':
        ...
        eval.add_argument('--prec-log', type=int, default=None if single_task else 500, metavar='N', help='# iters after which to plot precision')
    ...
    if compare_code == 'none' and generative:
            eval.add_argument('--sample-log', type=int, default=1000, metavar='N', help='# iters after which to plot samples')
    if generative:
        ...
        if (not only_MNIST) and compare_code=='bir':
            eval.add_argument('--eval-gen', action='store_true', help='instead of accuracy, evaluate quality of generators')

    return parser

def add_task_options(parser, only_MNIST=True, single_task=False, compare_code='none', **kwargs):
    ...
    if single_task:
        ...
        MNIST_tasks = ['splitMNIST', 'permMNIST', 'expandMNIST', 'singleMNIST']
    ...
    if not only_MNIST:
        task_params.add_argument('--augment', action='store_true', help='augment training data (random crop & horizontal flip)')
        task_params.add_argument('--no-norm', action='store_false', dest='normalize', help='don\'t normalize images (only for CIFAR)')
...
def add_model_options(parser, only_MNIST=True, generative=False, single_task=False, **kwargs):
    ...
    if not only_MNIST:
        ...
        model.add_argument('--n-blocks', type=int, default=1, help='# blocks per conv-layer (only for \'resNet\')')
        model.add_argument('--depth', type=int, default=5 if single_task else None, help='# of convolutional layers (0 = only fc-layers)')
        ...
        model.add_argument('--conv-bn', type=str, default='no', help='use batch-norm in the conv-layers (yes|no)')
        model.add_argument('--conv-nl', type=str, default='relu', choices=['relu', 'leakyrelu'])
        model.add_argument('--global-pooling', action='store_true', dest='gp', help='have global pool after conv-layers')
    # -fully-connected-layers
    model.add_argument('--fc-layers', type=int, default=3, dest='fc_lay', help='# of fully-connected layers')
    model.add_argument('--fc-units', type=int, default=2000 if single_task else None, metavar='N', help='# of units in first fc-layers')
...
def add_train_options(parser, only_MNIST=True, single_task=False, generative=False, compare_code='none', **kwargs):
    # training hyperparameters / initialization
    train_params = parser.add_argument_group('Training Parameters')
    if single_task:
        iter_epochs = train_params.add_mutually_exclusive_group(required=False)
        iter_epochs.add_argument('--epochs', type=int, default=10, metavar='N', help='max # of epochs (default: %(default)d)')
    ...
    train_params.add_argument('--batch', type=int, default=64 if single_task else None, help='batch-size')
    ...
    if not only_MNIST:
        ...
        train_params.add_argument('--convE-ltag', type=str, metavar='LTAG', default='none', help='tag for loading convE-layers')
...
def add_VAE_options(parser, only_MNIST=True,  **kwargs):
...
def add_replay_options(parser, only_MNIST=True, compare_code='none', **kwargs):
...
def add_bir_options(parser, only_MNIST=True, compare_code='none', **kwargs):
...
def add_snn_options(parser, compare_code='none', **kwargs):
    snn = parser.add_argument_group('SNNs Parameters')
    snn.add_argument('--gpu', type=int, default=-1, help='which gpu to use')

    snn.add_argument('--snn', action='store_true', help='use SNNs')
    snn.add_argument('--classes', type=int, default=10, help='total classes')
    snn.add_argument('--time_window', type=int, default=20, help='number of time window for each sample')

    snn.add_argument('--mask_mode', type=str, default='label', choices=['label', 'task'])
    snn.add_argument('--masking', action='store_true', help='use STDP masks')
    snn.add_argument('--hard_masking', action='store_true', help='use a threshold to rewrite STDP masks')
    snn.add_argument('--na_threshold', type=float, default=100.0, help='hard masking threshold')
    snn.add_argument('--stdp_threshold', type=float, default=100.0, help='hard masking threshold')
    snn.add_argument('--na_update_rate', type=float, default=0.5, help='rate while updating NA masks')
    snn.add_argument('--stdp_update_rate', type=float, default=0.5, help='rate while updating STDP masks')
    snn.add_argument('--calculate_with_firerate', action='store_true', help='use firerate to calculate')
    snn.add_argument('--current_multiple', type=float, default=1.0, help='multiple of current after sigmoid')
    snn.add_argument('--current_bias', type=float, default=0, help='bias of current after sigmoid')
    snn.add_argument('--use_stp', action='store_true', help='use stp')
    snn.add_argument('--stp_decay', type=float, default=0.2)

    snn.add_argument('--train_all_position', action='store_true', help='train all output positions')
    snn.add_argument('--change_order', action='store_true', help='change the task order')
    snn.add_argument('--task_order', nargs='+', type=int, help='task order')

    snn.add_argument('--trace_replay', action='store_true', help='generate replay with trace')

    snn.add_argument('--use_op', action='store_true')
    snn.add_argument('--op', type=str, default='in', choices=['in', 'out'])
...
def set_defaults(args, only_MNIST=True, single_task=False, generative=True, compare_code='None', **kwargs):
    ...
    if hasattr(args, 'recon_loss'):
        args.recon_loss = ('MSE' if args.experiment in ('CIFAR10', 'CIFAR100') else 'BCE') if args.recon_loss is None else args.recon_loss
    ...
    if not single_task:
        args.tasks= (5 if args.experiment == 'splitMNIST' else (10 if args.experiment == 'CIFAR100' else 100)) if args.tasks is None else args.tasks
        ...
        args.batch = (128 if args.experiment in ['splitMNIST', 'expandMNIST', 'singleMNIST'] else 256) if args.batch is None else args.batch
    ...
    if not single_task and not compare_code in ('hyper', 'bir'):
        if args.experiment == 'splitMNIST':
            ...
            args.ewc_lambda = (1000000000. if args.scenario == 'task' else 100000.) if args.ewc_lambda is None else args.ewc_lambda
