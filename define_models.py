...
def define_autoencoder(args, config, device, generator=False, convE=None):
    ...
    from models.vae import AutoEncoder, spiking_AutoEncoder
    # -create model
    if args.snn:
        if (hasattr(args, 'depth') and args.depth > 0):
            model = spiking_AutoEncoder(args,
...
def init_params(model, args):
    ...
    if utils.checkattr(args, 'pre_convE') and hasattr(model, 'depth') and model.depth > 0:
        load_name = model.convE.name if not hasattr(args, 'convE_ltag') or args.convE_ltag == 'none' else '{}-{}'.format(model.convE.name, args.convE_ltag)
