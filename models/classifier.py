...
from models.fc.layers import fc_layer, spiking_fc_layer
from models.fc.nets import MLP, spiking_MLP
...
class Classifier(ContinualLearner):
    ...
    def __init__(self, args, image_size, image_channels, classes,
                 # -conv-layers
                 conv_type='standard', depth=0, start_channels=64, reducing_layers=3, conv_bn=True, conv_nl='relu', num_blocks=2, global_pooling=False, no_fnl=False, conv_gated=False,
                 # -fc-layers
                 fc_layers=3, fc_units=1000, h_dim=400, fc_drop=0, fc_bn=True, fc_nl='relu', fc_gated=False, bias=False, excitability=False, excit_buffer=False,
        ...
        #------------------------------------------------------------------------------------------#
        if args.snn or spiking:
            #--> fully connected layers
            self.fcE = spiking_MLP(args, size_per_layer=self.fc_layer_sizes, drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, bias=bias, excitability=excitability, excit_buffer=excit_buffer, gated=fc_gated) #, output='none') ## NOTE: temporary change!!!
            #--> classifier
            self.classifier = spiking_fc_layer(args, self.units_before_classifier, classes, excit_buffer=True, nl='none', drop=fc_drop)
        else:
            #--> fully connected layers
            self.fcE = MLP(size_per_layer=self.fc_layer_sizes, drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, bias=bias, excitability=excitability, excit_buffer=excit_buffer, gated=fc_gated) #, output='none') ## NOTE: temporary change!!!
            #--> classifier
            self.classifier = fc_layer(self.units_before_classifier, classes, excit_buffer=True, nl='none', drop=fc_drop)

        self.experiment = args.experiment
        self.scenario = args.scenario if hasattr(args, 'scenario') else None
        self.snn = args.snn
        self.change_order = args.change_order
        self.task_order = args.task_order
        self.train_all_position = args.train_all_position
    ...
    def name(self):
        if self.depth > 0 and self.fc_layers > 1:
            ...
            return 'i{}_{}c{}'.format(self.fc_layer_sizes[0], 'drop{}-'.format(self.fc_drop) if self.fc_drop > 0 else '', self.classes)

    def forward(self, x, labels=None, task=None):
        hidden_rep = self.convE(x)
        final_features = self.fcE(self.flatten(hidden_rep), labels, task)

        return self.classifier(final_features, labels, task)

    def input_to_hidden(self, x):
        '''Get [hidden_rep]s (inputs to final fully-connected layers) for images [x].'''
        return self.convE(x)

    def hidden_to_output(self, hidden_rep, labels=None, task=None):
        '''Map [hidden_rep]s to outputs (i.e., logits for all possible output-classes).'''
        return self.classifier(self.fcE(self.flatten(hidden_rep), labels, task), labels, task)
    ...
    def classify(self, x, labels=None, task=None, not_hidden=False):
        ...
        hE = self.fcE(image_features, labels, task)

        return self.classifier(hE, labels, task)

    def orthogonal_loss(self, active_classes):
        total_orthogonal_loss = 0
        if type(active_classes[0]) == list:
            active_classes = active_classes[0]

        time_window = self.fcE.fcLayer1.linear.time_window
        synaptic_activity = []
        synaptic_activity.append(torch.bmm(self.fcE.fcLayer1.linear.sum_spike.unsqueeze(2) / time_window, self.x.view(self.x.shape[0], 1, -1)).mean(dim=0))
        synaptic_activity.append(torch.bmm(self.fcE.fcLayer2.linear.sum_spike.unsqueeze(2) / time_window, self.fcE.fcLayer1.linear.sum_spike.unsqueeze(1) / time_window).mean(dim=0))
        synaptic_activity.append(torch.bmm(self.classifier.linear.sum_spike.unsqueeze(2) / time_window, self.fcE.fcLayer2.linear.sum_spike.unsqueeze(1) / time_window).mean(dim=0))

        for i in range(len(self.classifier.linear.stdp_list)):
            if i not in active_classes:
                total_orthogonal_loss = total_orthogonal_loss + (F.cosine_similarity(synaptic_activity[0].view(1, synaptic_activity[0].shape[0] * synaptic_activity[0].shape[1]), self.fcE.fcLayer1.linear.stdp_list[i].view(1, synaptic_activity[0].shape[0] * synaptic_activity[0].shape[1])) - 0.9).pow(2)
                total_orthogonal_loss = total_orthogonal_loss + (F.cosine_similarity(synaptic_activity[1].view(1, synaptic_activity[1].shape[0] * synaptic_activity[1].shape[1]), self.fcE.fcLayer2.linear.stdp_list[i].view(1, synaptic_activity[1].shape[0] * synaptic_activity[1].shape[1])) - 0.9).pow(2)
                # total_orthogonal_loss = total_orthogonal_loss + (F.cosine_similarity(synaptic_activity[2].view(1, synaptic_activity[2].shape[0] * synaptic_activity[2].shape[1]), self.classifier.linear.stdp_list[i].view(1, synaptic_activity[2].shape[0] * synaptic_activity[2].shape[1])) - 0.5).pow(2)

        return total_orthogonal_loss

    def train_a_batch(self, x, y_ori, y=None, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, task=1, replay_not_hidden=False, freeze_convE=False, **kwargs):
        ...
        if x is not None:
            ...
            if y.shape.__len__() == 0:
                y = y.unsqueeze(0)

            # Run model
            # y_hat = self(x, torch.zeros(y.shape[0], 10).cuda().scatter(1, y.unsqueeze(1), 1))
            if self.scenario == 'task':
                if self.experiment == 'expandMNIST':
                    y_new = y + task - 1
                else:
                    y_new = y
            ...
            if self.snn:
                predL = None if y is None else F.mse_loss(input=y_hat, target=torch.zeros(y_hat.shape[0], y_hat.shape[1]).cuda().scatter(1, y.unsqueeze(1), 1), reduction='none') # --> no reduction needed, summing over classes is 'implicit'
            ...
            # Weigh losses
            loss_cur = predL # + self.orthogonal_loss(active_classes) * 0.01
        ...
        if x_ is not None:
            ...
            for replay_id in range(n_replays):
                if self.experiment == 'expandMNIST':
                    y_new_ = y_[replay_id] + replay_id
                elif self.scenario in ['task']:
                    y_new_ = y_[replay_id] + replay_id * 2
                else:
                    y_new_ = y_
                ...
                if (type(x_) == list) or (self.mask_dict is not None):
                    ...
                    y_hat_all = self.classify(x_temp_, y_new_, replay_id, not_hidden=replay_not_hidden)
                ...
                if (y_ is not None) and (y_[replay_id] is not None):
                    if self.snn:
                        predL_r[replay_id] = F.mse_loss(input=y_hat, target=torch.zeros(y_hat.shape[0], y_hat.shape[1]).cuda().scatter(1, y_[replay_id].unsqueeze(1), 1), reduction='none')
                ...
                if (scores_ is not None) and (scores_[replay_id] is not None):
                    ...
                    distilL_r[replay_id] = lf.loss_fn_kd(scores=y_hat[:, :n_classes_to_consider], target_scores=scores_[replay_id], T=self.KD_temp,)  # --> summing over classes & averaging over batch within this function
                # Weigh losses
