...
from models.fc.nets import MLP, MLP_gates, spiking_MLP, spiking_MLP_gates
from models.fc.layers import fc_layer,fc_layer_split, fc_layer_fixed_gates, spiking_fc_layer, spiking_fc_layer_split, spiking_fc_layer_fixed_gates
...
class AutoEncoder(ContinualLearner):
    ...
    def __init__(self, image_size, image_channels, classes,
                 # -conv-layers
                 conv_type='standard', depth=0, start_channels=64, reducing_layers=3, conv_bn=True, conv_nl='relu', num_blocks=2, global_pooling=False, no_fnl=True, convE=None, conv_gated=False,
                 # -fc-layers
                 fc_layers=3, fc_units=1000, h_dim=400, fc_drop=0, fc_bn=False, fc_nl='relu', excit_buffer=False, fc_gated=False,
                 # -prior
                 prior='standard', z_dim=20, per_class=False, n_modes=1,
                 # -decoder
                 recon_loss='BCE', network_output='sigmoid', deconv_type='standard', hidden=False, dg_gates=False, dg_type='task', dg_prop=0., tasks=5, scenario='task', device='cuda',
                 # -classifer
        ...
        ##>----Encoder (= q[z|x])----<##
        self.convE = ConvLayers(conv_type=conv_type, block_type='basic', num_blocks=num_blocks, image_channels=image_channels, depth=self.depth, start_channels=start_channels, reducing_layers=reducing_layers, batch_norm=conv_bn, nl=conv_nl, output='none' if no_fnl else 'normal', global_pooling=global_pooling, gated=conv_gated) if (convE is None) else convE
        ...
        #------------------------------------------------------------------------------------------#
        self.fcE = MLP(size_per_layer=self.fc_layer_sizes, drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, excit_buffer=excit_buffer, gated=fc_gated)
        # to z
        ...
        if self.dg_gates:
            self.fcD = MLP_gates(size_per_layer=[x for x in reversed(fc_layer_sizes_down)], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, gate_size=self.gate_size, gating_prop=dg_prop, device=device, output=self.network_output if (self.depth == 0 or self.hidden) else 'normal',)
        else:
            self.fcD = MLP(size_per_layer=[x for x in reversed(fc_layer_sizes_down)], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, gated=fc_gated, output=self.network_output if (self.depth == 0 or self.hidden) else 'normal',)
        # to image-shape
        ...
        self.convD = DeconvLayers(image_channels=image_channels, final_channels=start_channels, depth=self.depth, reducing_layers=reducing_layers, batch_norm=conv_bn, nl=conv_nl, gated=conv_gated, output=self.network_output, deconv_type=deconv_type,) if (not self.hidden) else modules.Identity()
    ...
    def get_name(self):
        ...
        fcE_label = '{}_'.format(self.fcE.name) if self.fc_layers > 1 else '{}{}_'.format('h' if self.depth > 0 else 'i', self.conv_out_units)
        z_label = 'z{}{}'.format(self.z_dim, '' if self.prior == 'standard' else '-{}{}{}'.format(self.prior, self.n_modes, 'pc' if self.per_class else ''))
        class_label = '_c{}{}'.format(self.classes, '' if self.classify_opt == 'beforeZ' else self.classify_opt) if hasattr(self, 'classifier') else ''
    ...
    def sample(self, size, allowed_classes=None, class_probs=None, sample_mode=None, allowed_domains=None, only_x=False, **kwargs):
        ...
        if self.dg_gates and self.dg_type == 'task':
            if self.scenario == 'domain':
                task_used = np.random.randint(0,self.gate_size,size) if (allowed_domains is None) else np.random.choice(allowed_domains, size, replace=True)
            else:
    ...
    def loss_function(self, x, y, x_recon, y_hat, scores, mu, z, logvar=None, allowed_classes=None, batch_weights=None):
        ...
        ###-----Reconstruction loss-----###
        batch_size = x.size(0)
        reconL = self.calculate_recon_loss(x=x.view(batch_size, -1), average=True, x_recon=x_recon.view(batch_size, -1)) # -> average over pixels
        reconL = lf.weighted_average(reconL, weights=batch_weights, dim=0)       # -> average over batch

        ###-----Variational loss-----###
        if logvar is not None:
            actual_y = torch.tensor([allowed_classes[i.item()] for i in y]).to(self._device()) if ((allowed_classes is not None) and (y is not None)) else y
        ...
        if y is not None and y_hat is not None:
            ...
            predL = F.mse_loss(input=y_hat, target=torch.zeros(y_hat.shape[0], y_hat.shape[1]).cuda().scatter(1, y.unsqueeze(1), 1), reduction='none')
        ...
        if scores is not None and y_hat is not None:
            ...
            distilL = lf.loss_fn_kd(scores=y_hat[:, :n_classes_to_consider], target_scores=scores, T=self.KD_temp, weights=batch_weights)  #--> summing over classes & averaging over batch in function
        else:
    ...
    def calculate_recon_error(self, dataset, batch_size=128, max_batches=None, average=False):
        ...
        if self.scenario == 'task' or (self.dg_gates and self.dg_prop > 0. and self.dg_type == 'task'):
            raise NotImplementedError('Function \'calculate_recon_error\' not yet implemented for Task-IL scenario or task-based decoder-gates')
    ...
    def estimate_loglikelihood(self, dataset, S=5000, batch_size=128, max_n=None):
        ...
        # This function currently does not (always) work for Task-IL scenario or for decoder-gates with [dg_type]='task'
        if self.scenario == 'task' or (self.dg_gates and self.dg_prop > 0. and self.dg_type == 'task'):
            raise NotImplementedError('Function \'estimate_loglikelihood\' not yet implemented for Task-IL scenario or task-based decoder-gates')
    ...
    def train_a_batch(self, x, y=None, x_=None, y_=None, scores_=None, tasks_=None, rnt=0.5, active_classes=None, task=1, replay_not_hidden=False, freeze_convE=False, **kwargs):
        ...
        if x is not None:
            ...
            recon_batch, y_hat, mu, logvar, z = self(x, gate_input=(task_tensor if self.dg_type == 'task' else y) if self.dg_gates else None, full=True, reparameterize=True)
                ...
                if y_hat is not None:
                    if type(y_hat) == list:
                        ...
                        for i in range(len(y_hat)):
                            spikes += y_hat[i]
                        y_hat = spikes / len(y_hat)
                    y_hat = y_hat[:, class_entries]

            # Calculate all losses
            reconL, variatL, predL, _ = self.loss_function(x=x, y=y, x_recon=recon_batch, y_hat=y_hat, scores=None, mu=mu, z=z, logvar=logvar, allowed_classes=class_entries if active_classes is not None else None) #--> [allowed_classes] will be used only if [y] is not provided
            ...
            if (self.mask_dict is not None) and (x_ is not None):
                ...
                weighted_current_loss.backward(retain_graph=True)
        ...
        if x_ is not None:
            ...
            for replay_id in range(n_replays):
                ...
                if type(y_hat_all) == list:
                    spikes = 0
                    for i in range(len(y_hat_all)):
                        spikes += y_hat_all[i]
                    y_hat_all = spikes / len(y_hat_all)
                ...
                if self.mask_dict is not None:
                    ...
                    weighted_replay_loss_this_task.backward(retain_graph=True)
...
class spiking_AutoEncoder(ContinualLearner):
    '''Class for variational auto-encoder (VAE) models.'''

    def __init__(self, args, image_size, image_channels, classes,
                 # -conv-layers
                 conv_type='standard', depth=0, start_channels=64, reducing_layers=3, conv_bn=True, conv_nl='relu', num_blocks=2, global_pooling=False, no_fnl=True, convE=None, conv_gated=False,
                 # -fc-layers
                 fc_layers=3, fc_units=1000, h_dim=400, fc_drop=0, fc_bn=False, fc_nl='relu', excit_buffer=False, fc_gated=False,
        ...
        real_h_dim = h_dim if fc_layers > 1 else self.conv_out_units
        #------------------------------------------------------------------------------------------#
        self.fcE = spiking_MLP(args, size_per_layer=self.fc_layer_sizes, drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, excit_buffer=excit_buffer, gated=fc_gated)
        # to z
        self.toZ = spiking_fc_layer_split(args, real_h_dim, z_dim, nl_mean='none', nl_logvar='none')#, drop=fc_drop)

        ##>----Classifier----<##
        if classifier:
            self.units_before_classifier = real_h_dim if self.classify_opt == 'beforeZ' else z_dim
            self.classifier = spiking_fc_layer(args, self.units_before_classifier, classes, excit_buffer=True, nl='none')

        ##>----Decoder (= p[x|z])----<##
        out_nl = True if fc_layers > 1 else (True if (self.depth > 0 and not no_fnl) else False)
        real_h_dim_down = h_dim if fc_layers > 1 else self.convE.out_units(image_size, ignore_gp=True)
        if self.dg_gates:
            self.fromZ = spiking_fc_layer_fixed_gates(args, z_dim, real_h_dim_down, batch_norm=(out_nl and fc_bn), nl=fc_nl if out_nl else 'none', gate_size=self.gate_size, gating_prop=dg_prop, device=device)
        else:
            self.fromZ = spiking_fc_layer(args, z_dim, real_h_dim_down, batch_norm=(out_nl and fc_bn), nl=fc_nl if out_nl else 'none')
        fc_layer_sizes_down = self.fc_layer_sizes
        fc_layer_sizes_down[0] = self.convE.out_units(image_size, ignore_gp=True)
        # -> if 'gp' is used in forward pass, size of first/final hidden layer differs between forward and backward pass
        if self.dg_gates:
            self.fcD = spiking_MLP_gates(args, size_per_layer=[x for x in reversed(fc_layer_sizes_down)], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, gate_size=self.gate_size, gating_prop=dg_prop, device=device, output=self.network_output if (self.depth == 0 or self.hidden) else 'normal',)
        else:
            self.fcD = spiking_MLP(args, size_per_layer=[x for x in reversed(fc_layer_sizes_down)], drop=fc_drop, batch_norm=fc_bn, nl=fc_nl, gated=fc_gated, output=self.network_output if (self.depth == 0 or self.hidden) else 'normal',)
        # to image-shape
        self.to_image = modules.Reshape(image_channels=self.convE.out_channels if self.depth > 0 else image_channels)
        # through deconv-layers
        self.convD = DeconvLayers(image_channels=image_channels, final_channels=start_channels, depth=self.depth, reducing_layers=reducing_layers, batch_norm=conv_bn, nl=conv_nl, gated=conv_gated, output=self.network_output, deconv_type=deconv_type,) if (not self.hidden) else modules.Identity()
    ...
    def decode(self, z, gate_input=None):
        ...
        if type(image_features) == list:
            ...
            for i in range(len(image_features)):
                spikes += image_features[i]
            image_features = spikes / len(image_features)
