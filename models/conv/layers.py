...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, batch_norm=True, nl='relu', no_fnl=False):
        ...
        self.block_layer = nn.Sequential(
            ...
            nn.Conv2d(planes, self.expansion * planes, kernel_size=3, stride=1, padding=1, bias=False if batch_norm else True),
        )
        # shortcut block-layer
        self.shortcut = modules.Identity()
        if stride != 1 or in_planes != self.expansion * planes:
            self.shortcut = nn.Sequential(
                nn.Conv2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, bias=False if batch_norm else True),
...
class conv_layer_split(nn.Module):
    ...
    def __init__(self, in_planes, out_planes, nl_mean=nn.ReLU(), nl_logvar=nn.Hardtanh(),
        ...
        super().__init__()
        self.mean = conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, drop=drop, batch_norm=batch_norm, bias=bias, gated=gated, nl=nl_mean)
        self.logvar = conv_layer(in_planes, out_planes, kernel_size=kernel_size, stride=stride, padding=padding, drop=drop, batch_norm=batch_norm, bias=False, gated=gated, nl=nl_logvar)
...
class DeconvBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1, batch_norm=True, nl='relu', no_fnl=False, smaller_kernel=False):
        ...
        # normal block-layers
        self.block_layer1 = nn.Sequential(
            nn.ConvTranspose2d(in_planes, planes, stride=stride, bias=False if batch_norm else True, kernel_size=(2 if smaller_kernel else 4) if stride == 2 else 3, padding=0 if (stride == 2 and smaller_kernel) else 1),
            nn.ReLU() if nl == 'relu' else nn.LeakyReLU()
        )
        self.block_layer2 = nn.Sequential(
            nn.ConvTranspose2d(planes, self.expansion * planes, kernel_size=3, stride=1, padding=1, bias=False if batch_norm else True),
        )
        ...
        if stride != 1 or in_planes != self.expansion*planes:
            self.shortcut = nn.Sequential(
                nn.ConvTranspose2d(in_planes, self.expansion * planes, kernel_size=1, stride=stride, output_padding=0 if stride == 1 else 1, bias=False if batch_norm else True),
                nn.BatchNorm2d(self.expansion * planes) if batch_norm else modules.Identity()
            )
