...
from models.fc.layers import fc_layer, fc_layer_fixed_gates, spiking_fc_layer, spiking_fc_layer_fixed_gates
...
class MLP(nn.Module):
    ...
    def __init__(self, input_size=1000, output_size=10, layers=2, hid_size=1000, hid_smooth=None, size_per_layer=None, drop=0, batch_norm=True, nl='relu', bias=True, excitability=False, excit_buffer=False, gated=False, output='normal'):
        ...
        nd_label = '{drop}{bias}{exc}{bn}{nl}{gate}'.format(drop='' if drop==0 else 'd{}'.format(drop), bias='' if bias else 'n', exc='e' if excitability else '', bn='b' if batch_norm else '', nl='l' if nl=='leakyrelu' else '', gate='g' if gated else '',)
        ...
        for lay_id in range(1, self.layers + 1):
            ...
            layer = fc_layer(in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False if (lay_id == self.layers and not output == 'normal') else batch_norm, gated=gated, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
    ...
    def forward(self, x, labels=None, task=None, skip_first=0, skip_last=0, return_lists=False, **kwargs):
...
class spiking_MLP(nn.Module):
    ...
    def __init__(self, args, input_size=1000, output_size=10, layers=2, hid_size=1000, hid_smooth=None, size_per_layer=None, drop=0, batch_norm=False, nl='relu', bias=True, excitability=False, excit_buffer=False, gated=False, output='normal'):
        '''sizes: 0th=[input], 1st=[hid_size], ..., 1st-to-last=[hid_smooth], last=[output].
        [input_size]       # of inputs
        [output_size]      # of units in final layer
        [layers]           # of layers
        ...
        for lay_id in range(1, self.layers + 1):
            ...
            out_size = size_per_layer[lay_id]
            # define and set the fully connected layer
            layer = spiking_fc_layer(args, in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False, gated=gated, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
            setattr(self, 'fcLayer{}'.format(lay_id), layer)
    ...
    def forward(self, x, labels=None, task=None, skip_first=0, skip_last=0, return_lists=False, **kwargs):
        ...
        # Sequentially pass [x] through all fc-layers
        for lay_id in range(skip_first + 1, self.layers + 1 - skip_last):
            (x, pre_act) = getattr(self, 'fcLayer{}'.format(lay_id))(x, labels, task, return_pa=True)
            if return_lists:
                pre_act_list.append(pre_act)  #-> for each layer, store pre-activations
                if lay_id < (self.layers - skip_last):
                    hidden_act_list.append(x) #-> for all but last layer, store hidden activations
...
class MLP_gates(nn.Module):
    ...
    def __init__(self, input_size=1000, output_size=10, layers=2, hid_size=1000, hid_smooth=None, size_per_layer=None, drop=0, batch_norm=True, nl='relu', bias=True, excitability=False, excit_buffer=False, gate_size=0, gating_prop=0., final_gate=False, output='normal', device='cuda'):
        '''sizes: 0th=[input], 1st=[hid_size], ..., 1st-to-last=[hid_smooth], last=[output].
        [input_size]       # of inputs
        [output_size]      # of units in final layer
        [layers]           # of layers
        ...
        # set label for this module
        # -determine 'non-default options'-label
        nd_label = '{drop}{bias}{exc}{bn}{nl}{gate}'.format(drop='' if drop == 0 else 'd{}'.format(drop), bias='' if bias else 'n', exc='e' if excitability else '', bn='b' if batch_norm else '', nl='l' if nl == 'leakyrelu' else '', gate='g{}m{}'.format(gate_size, gating_prop) if (gate_size > 0 and gating_prop > 0.) else '',)
        nd_label = '{}{}'.format('' if nd_label == '' else '-{}'.format(nd_label), '' if output == 'normal' else '-{}'.format(output))
        # -set label
        size_statement = ''
        for i in size_per_layer:
        ...
        for lay_id in range(1, self.layers + 1):
            ...
            if (not gate_size > 0.) or (not gating_prop > 0.) or (lay_id == self.layers and not final_gate):
                layer = fc_layer(in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False if (lay_id == self.layers and not output == 'normal') else batch_norm, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
            else:
                layer = fc_layer_fixed_gates(in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False if (lay_id == self.layers and not output == 'normal') else batch_norm, gate_size=gate_size, gating_prop=gating_prop, device=device, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
            setattr(self, 'fcLayer{}'.format(lay_id), layer)
    ...
    def forward(self, x, labels=None, task=None, gate_input=None, skip_first=0, skip_last=0, return_lists=False):
        # Initiate <list> for keeping track of intermediate hidden-(pre)activations
        if return_lists:
            hidden_act_list = []
            pre_act_list = []
        # Sequentially pass [x] through all fc-layers
        for lay_id in range(skip_first + 1, self.layers + 1 - skip_last):
            (x, pre_act) = getattr(self, 'fcLayer{}'.format(lay_id))(x, labels, task, gate_input=gate_input, return_pa=True)
            if return_lists:
                pre_act_list.append(pre_act)  #-> for each layer, store pre-activations
                if lay_id < (self.layers - skip_last):
                    hidden_act_list.append(x) #-> for all but last layer, store hidden activations
...
class spiking_MLP_gates(nn.Module):
    '''Module for a multi-layer perceptron (MLP). Possible to return (pre)activations of each layer.
    Also possible to supply a [skip_first]- or [skip_last]-argument to the forward-function to only pass certain layers.
    With gates controlled by [gate_input] (of size [gate_size]) with a randomly selected masked (prop=[gating_prop]).
    ...
    def __init__(self, args, input_size=1000, output_size=10, layers=2, hid_size=1000, hid_smooth=None, size_per_layer=None, drop=0, batch_norm=True, nl='relu', bias=True, excitability=False, excit_buffer=False, gate_size=0, gating_prop=0., final_gate=False, output='normal', device='cuda'):
        '''sizes: 0th=[input], 1st=[hid_size], ..., 1st-to-last=[hid_smooth], last=[output].
        [input_size]       # of inputs
        [output_size]      # of units in final layer
        [layers]           # of layers
        ...
        for lay_id in range(1, self.layers + 1):
            ...
            # define and set the fully connected layer
            if (not gate_size > 0.) or (not gating_prop > 0.) or (lay_id == self.layers and not final_gate):
                layer = spiking_fc_layer(args, in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False if (lay_id == self.layers and not output == 'normal') else batch_norm, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
            else:
                layer = spiking_fc_layer_fixed_gates(args, in_size, out_size, bias=bias, excitability=excitability, excit_buffer=excit_buffer, batch_norm=False if (lay_id == self.layers and not output == 'normal') else batch_norm, gate_size=gate_size, gating_prop=gating_prop, device=device, nl=('none' if output == 'none' else nn.Sigmoid()) if (lay_id == self.layers and not output == 'normal') else nl, drop=drop if lay_id > 1 else 0.,)
            setattr(self, 'fcLayer{}'.format(lay_id), layer)
