...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_CIFAR100_bir', description='Compare different components of BI-R on split CIFAR-100.')
...
def barplots(dict1, dict2, names1, names2, ids, colors, seed_list, index, only_last_dir=None, chance_level=None, long_names1=None, long_names2=None, ylabel=None, title=None, ylim=None, perc=False, neg=False):
    ...
    # bar-plot
    figure = plt.plot_bars([means1, means2], names=[names1, names2], colors=[colors, colors], ylabel=ylabel, yerr=[sems1, sems2] if len(seed_list) > 1 else None, ylim=ylim, top_title=title, title_list=['Additions to Standard GR', 'Ablations from BI-R'], vlines=[0.5, 0.5], alpha=[0.7, 1], dots=[dots1, dots2] if len(seed_list) > 1 else None, h_line=chance_level, h_label='chance level' if chance_level is not None else None, h_lines=h_lines, h_errors=h_errors, h_labels=h_labels, h_colors=h_colors)
...
def pr_curves(result_dict, ids, seed_list, colors, names, i_recall, i_precision, title_top=None, title=None, alpha=None):
    ...
    figure = plt.plot_pr_curves(precision_list, recall_list, names=names, colors=colors, with_dots=False, linestyle='solid', title=title, title_top=title_top, alpha=alpha)
...
if __name__ == '__main__':
    ...
    args.dg_prop = 0.
    BIR_nG = {}
    BIR_nG = collect_all(BIR_nG, seed_list, args, name='BI-R: no gating based on internal context', no_gen=(not args.eval_gen))
    args.dg_gates = True
    ...
    long_names_bir = ['BI-R', '- replay-through-feedback', '- conditional replay', '- gating', '- internal replay', '- distillation']
    names_sg = ['s-GR', '+ rtf', '+ con', '+ gat', '+ int', '+ dis']
    long_names_sg = ['s-GR', '+ replay-through-feedback', '+ conditional replay', '+ gating', '+ internal replay',  '+ distillation']
    ...
    if not args.eval_gen:
        ...
        print('\n\n{}\n'.format(title) + '-' * 60)
        figure = barplots(ALL_sg, ALL_bir, names_sg, names_bir, ids, colors, seed_list, index, only_last_dir=ONLYLAST, chance_level=chance_level, long_names1=long_names_sg, long_names2=long_names_bir, ylabel=ylabel, title=title, ylim=None, perc=True, neg=False)
        figure_list.append(figure)
    else:
        ##--- LOG LIKELIHOOD ---##
        index = 1
        ylabel = 'Average test negative log-likelihood (over all {})'.format('{} classes'.format(total_classes) if args.scenario == 'class' else '{} tasks'.format(args.tasks))
        title = 'NEGATIVE LOG-LIKELIHOOD'
        print('\n\n{}\n'.format(title) + '-' * 60)
        figure = barplots(ALL_sg, ALL_bir, names_sg, names_bir, ids, colors, seed_list, index, long_names1=long_names_sg, long_names2=long_names_bir, ylabel=ylabel, title=title, ylim=None, perc=False, neg=True)
        figure_list.append(figure)

        ##--- RECONSTRUCTION ERROR ---##
        index = 2
        ylabel = 'Average test reconstruction error (MSE), divided by number of units (over all {})'.format('{} classes'.format(total_classes) if args.scenario == 'class' else '{} tasks'.format(args.tasks))
        title = 'RECONSTRUCTION ERROR (PER INPUT UNIT)'
        print('\n\n{}\n'.format(title) + '-' * 60)
        figure = barplots(ALL_sg, ALL_bir, names_sg, names_bir, ids, colors, seed_list, index, long_names1=long_names_sg, long_names2=long_names_bir, ylabel=ylabel, title=title, ylim=None, perc=False, neg=False)
        figure_list.append(figure)
        ...
        fig = pr_curves(ALL_sg, ids, seed_list, colors, names_sg, i_recall=6, i_precision=5, title_top=title, title='Additions to Standard GR', alpha=0.7)
        figure_list.append(fig)
        fig = pr_curves(ALL_bir, ids, seed_list, colors, names_bir, i_recall=6, i_precision=5, title_top=title, title='Ablations from BI-R')
        figure_list.append(fig)
