...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_MNIST', description='Compare performance of various continual learning strategies on different scenarios of splitMNIST.')
...
if __name__ == '__main__':
    ...
    for seed in seed_list:
        i = 0
        prec[seed] = [OFF[seed][i]['average'], NONE[seed][i]['average'], LWF[seed][i]['average'], GR[seed][i]['average'], EWC[seed][i]['average'], SI[seed][i]['average'],]
        i = 1
        ave_prec[seed] = [OFF[seed][i], NONE[seed][i], LWF[seed][i], GR[seed][i], EWC[seed][i], SI[seed][i],]
        if args.scenario == 'task':
    ...
    figure = plt.plot_bar(means, names=names, colors=colors, ylabel=ylabel_all, title=title, yerr=sems if args.n_seeds > 1 else None, ylim=(0,1))
    ...
    figure = plt.plot_lines(ave_lines, x_axes=[2 * i for i in x_axes] if args.scenario == 'class' else x_axes, line_names=names, colors=colors, title=title, xlabel='# {} so far'.format('digits' if args.scenario == 'class' else 'tasks'), ylabel=ylabel, list_with_errors=sem_lines if args.n_seeds>1 else None, ylim=ylim)
    figure_list.append(figure)
