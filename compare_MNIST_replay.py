...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_MNIST_replay', description='Generative replay: effect of quantity & quality.')
...
if __name__ == '__main__':
    ...
    for batch_r in batch_r_list:
        ...
        GR_b[batch_r] = {}
        GR_b[batch_r] = collect_all(GR_b[batch_r], seed_list, args, name='GR - batch-size replay = {}'.format(batch_r))
    ...
    for batch_r in batch_r_list:
        args.batch_replay = batch_r
        GR_br[batch_r] = {}
        GR_br[batch_r] = collect_all(GR_br[batch_r], seed_list, args, name='GR & reinit - batch-size replay = {}'.format(batch_r))
    args.reinit = False
    args.batch_replay = None
    ...
    for g_fc in g_fc_uni_list:
        ...
        args.g_h_dim = g_fc
        GR_g[g_fc] = {}
        GR_g[g_fc] = collect_all(GR_g[g_fc], seed_list, args, name='GR - # units per hidden layer VAE = {}'.format(g_fc))
    ...
    for g_fc in g_fc_uni_list:
        ...
        args.g_h_dim = g_fc
        GR_gr[g_fc] = {}
        GR_gr[g_fc] = collect_all(GR_gr[g_fc], seed_list, args, name='GR & reinit - # units per hidden layer VAE = {}'.format(g_fc))
    args.reinit = False
    ...
    figure = plt.plot_lines_with_baselines(averages, x_axes=batch_r_list, ylabel='Test accuracy (after all tasks)', title=title, x_log=True, ylim=y_lim, line_names=line_names, xlabel='Replay batch-size (log-scale)', with_dots=True, list_with_errors=sems if args.n_seeds > 1 else None, h_lines=h_lines, h_errors=h_errors, h_labels=h_labels, h_colors=h_colors, colors=colors)
    ...
    figure = plt.plot_lines_with_baselines(averages, x_axes=g_fc_uni_list, ylabel='Test accuracy (after all tasks)', title=title, x_log=True, ylim=y_lim, line_names=line_names, xlabel='# of units in hidden layers VAE (log-scale)', with_dots=True, list_with_errors=sems if args.n_seeds > 1 else None, h_lines=h_lines, h_errors=h_errors, h_labels=h_labels, h_colors=h_colors, colors=colors)
    figure_list.append(figure)
