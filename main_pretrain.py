...
def handle_inputs():
    ...
    kwargs = {'single_task': True, 'only_MNIST': False, 'generative': False, 'compare_code': 'none', 'train_options': 'all'}
...
def run(args):
    ...
    print('\nPreparing the data...')
    (trainset, testset), config = get_singletask_experiment(name=args.experiment, data_dir=args.d_dir, verbose=True, normalize = True if utils.checkattr(args, 'normalize') else False, augment = True if utils.checkattr(args, 'augment') else False,)
    ...
    # Specify model
    if (utils.checkattr(args, 'pre_convE') or utils.checkattr(args, 'pre_convD')) and (hasattr(args, 'depth') and args.depth > 0):
        print('\nDefining the model...')
    ...
    latent_space_cb = cb._latent_space_cb(log=min(5 * eval_log, iters), datasets=[testset], visdom=visdom, sample_size=400)
    ...
    # (Pre)train model
    print('\nTraining...')
    train.train(cnn, train_loader, iters, loss_cbs=loss_cbs, eval_cbs=[eval_cb, latent_space_cb], save_every=1000 if args.save else None, m_dir=args.m_dir, args=args)

    # Save (pre)trained model
    if args.save:
        # -conv-layers
        save_name = cnn.convE.name if not hasattr(args, 'convE_stag') or args.convE_stag == 'none' else '{}-{}'.format(cnn.convE.name, args.convE_stag)
        utils.save_checkpoint(cnn.convE, args.m_dir, name=save_name)
        # -full model
        save_name = cnn.name if not hasattr(args, 'full_stag') or args.full_stag == 'none' else '{}-{}'.format(cnn.name, args.full_stag)
    ...
    if args.pdf:
        ...
        # -Fig2: precision
        figure = plt.plot_lines(precision_dict['all_tasks'], x_axes=precision_dict['x_iteration'], line_names=['ave precision'], xlabel='Iterations', ylabel='Test accuracy')
        pp.savefig(figure)
        # -close pdf
        pp.close()
