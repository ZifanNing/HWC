...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_permMNIST100', description='Compare performance of continual learning strategies on long instance of standard version (i.e., Domain-IL) of permMNIST (100 tasks).')
...
if __name__ == '__main__':
    ...
    names = ['None', 'LwF', 'EWC', 'Online EWC', 'SI', 'Generative Replay (GR)', 'Brain-Inspired Replay (BI-R)', 'BI-R + SI', 'Joint']
    ...
    figure = plt.plot_bar(means, names=names, colors=colors, ylabel='Test accuracy (after all 100 tasks)', title=title, yerr=sems if args.n_seeds > 1 else None, ylim=(0,1))
    ...
    ylim = (0.3, 1)
    figure = plt.plot_lines(ave_lines, x_axes=x_axes, line_names=names, colors=colors, title=title, xlabel='# of permutations so far', ylabel='Test accuracy (on tasks seen so far)', list_with_errors=sem_lines if args.n_seeds > 1 else None, ylim=ylim)
    figure_list.append(figure)
