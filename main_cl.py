...
def handle_inputs():
    ...
    kwargs = {'single_task': False, 'only_MNIST': True, 'generative': False, 'compare_code': 'none'}
    ...
    parser = options.add_snn_options(parser, **kwargs)
...
def run(args, verbose=False):
    ...
    if verbose and (utils.checkattr(args, 'pre_convE') or utils.checkattr(args, 'pre_convD')) and (hasattr(args, 'depth') and args.depth > 0):
        print('\nDefining the model...')
    ...
    if train_gen:
        ...
        # Set optimizer(s)
        generator.optim_list = [{'params': filter(lambda p: p.requires_grad, generator.parameters()), 'lr': args.lr_gen if hasattr(args, 'lr_gen') else args.lr},]
    ...
    if verbose:
        print('\nParameter-stamp...')
    param_stamp = get_param_stamp(args, model.name, verbose=verbose, replay=True if (hasattr(args, 'replay') and not args.replay == 'none') else False, replay_model_name=generator.name if (hasattr(args, 'replay') and args.replay in ('generative') and not utils.checkattr(args, 'feedback')) else None,)
    ...
    if args.visdom:
        ...
        replay_statement = '{mode}{fb}{con}{gat}{int}{dis}{b}{u}'.format(mode=args.replay, fb='Rtf' if utils.checkattr(args, 'feedback') else '', con='Con' if (hasattr(args, 'prior') and args.prior == 'GMM' and utils.checkattr(args, 'per_class')) else '', gat='Gat{}'.format(args.dg_prop) if (utils.checkattr(args, 'dg_gates') and hasattr(args, 'dg_prop') and args.dg_prop > 0) else '', int='Int' if utils.checkattr(args, 'hidden') else '', dis='Dis' if args.replay == 'generative' and args.distill else '', b='' if (args.batch_replay is None or args.batch_replay == args.batch) else '-br{}'.format(args.batch_replay), u='' if args.g_fc_uni == args.fc_units else '-gu{}'.format(args.g_fc_uni)) if (hasattr(args, 'replay') and not args.replay == 'none') else 'NR'
        graph_name = '{replay}{syn}{ewc}{xdg}'.format(replay=replay_statement, syn='-si{}'.format(args.si_c) if utils.checkattr(args, 'si') else '', ewc='-ewc{}{}'.format(args.ewc_lambda,'-O{}'.format(args.gamma) if utils.checkattr(args, 'online') else '') if utils.checkattr(args, 'ewc') else '', xdg='' if (not utils.checkattr(args, 'xdg')) or args.xdg_prop == 0 else '-XdG{}'.format(args.xdg_prop),)
    ...
    generator_loss_cbs = [cb._VAE_loss_cb(log=args.loss_log, visdom=visdom, replay=(hasattr(args, 'replay') and not args.replay == 'none'), model=model if utils.checkattr(args, 'feedback') else generator, tasks=args.tasks, iters_per_task=args.iters if utils.checkattr(args, 'feedback') else g_iters)] if (train_gen or utils.checkattr(args, 'feedback')) else [None]
    solver_loss_cbs = [cb._solver_loss_cb(log=args.loss_log, visdom=visdom, model=model, iters_per_task=args.iters, tasks=args.tasks, replay=(hasattr(args, 'replay') and not args.replay == 'none'))] if (not utils.checkattr(args, 'feedback')) else [None]

    # Callbacks for evaluating and plotting generated / reconstructed samples
    no_samples = (utils.checkattr(args, 'no_samples') or (utils.checkattr(args, 'hidden') and hasattr(args, 'depth') and args.depth > 0))
    sample_cbs = [cb._sample_cb(log=args.sample_log, visdom=visdom, config=config, test_datasets=test_datasets, sample_size=args.sample_n, iters_per_task=g_iters)] if ((train_gen or utils.checkattr(args, 'feedback')) and not no_samples) else [None]
    ...
    eval_cb = cb._eval_cb(log=args.prec_log, test_datasets=test_datasets, visdom=visdom, precision_dict=None, iters_per_task=args.iters, test_size=args.prec_n, classes_per_task=classes_per_task, scenario=args.scenario,)
    # -pdf / reporting: summary plots (i.e, only after each task)
    eval_cb_full = cb._eval_cb(log=args.iters, test_datasets=test_datasets, precision_dict=precision_dict, iters_per_task=args.iters, classes_per_task=classes_per_task, scenario=args.scenario,)
    # -visualize feature space
    latent_space_cb = cb._latent_space_cb(log=args.iters, datasets=test_datasets, visdom=visdom, iters_per_task=args.iters, sample_size=400,)
    ...
    if args.train:
        if verbose:
            print('\nTraining...')
        # Train model
        train_cl(model, train_datasets, replay_mode=args.replay if hasattr(args, 'replay') else 'none', scenario=args.scenario, classes_per_task=classes_per_task, iters=args.iters, batch_size=args.batch, batch_size_replay=args.batch_replay if hasattr(args, 'batch_replay') else None, generator=generator, gen_iters=g_iters, gen_loss_cbs=generator_loss_cbs, feedback=utils.checkattr(args, 'feedback'), sample_cbs=sample_cbs, eval_cbs=eval_cbs, loss_cbs=generator_loss_cbs if utils.checkattr(args, 'feedback') else solver_loss_cbs, args=args, reinit=utils.checkattr(args, 'reinit'), only_last=utils.checkattr(args, 'only_last'))
        ...
        if args.save:
            save_name = 'mM-{}'.format(param_stamp) if (not hasattr(args, 'full_stag') or args.full_stag == 'none') else '{}-{}'.format(model.name, args.full_stag)
            utils.save_checkpoint(model, args.m_dir, name=save_name, verbose=verbose)
            if generator is not None:
                save_name = 'gM-{}'.format(param_stamp) if (not hasattr(args, 'full_stag') or args.full_stag == 'none') else '{}-{}'.format(generator.name, args.full_stag)
        ...
        load_name = 'mM-{}'.format(param_stamp) if (not hasattr(args, 'full_ltag') or args.full_ltag == 'none') else '{}-{}'.format(model.name, args.full_ltag)
        utils.load_checkpoint(model, args.m_dir, name=load_name, verbose=verbose, add_si_buffers=(isinstance(model, ContinualLearner) and utils.checkattr(args, 'si')))
        if generator is not None:
            load_name = 'gM-{}'.format(param_stamp) if (not hasattr(args, 'full_ltag') or args.full_ltag == 'none') else '{}-{}'.format(generator.name, args.full_ltag)
    ...
    if args.scenario == 'task':
        if args.experiment == 'expandMNIST':
            task_label = [list(range(j, j + classes_per_task)) for j in range(args.tasks)]
        elif args.experiment == 'singleMNIST':
            task_label = [list(range(int((j // 2) * 2), int(((j // 2) + 1) * 2))) for j in range(args.tasks)]
        else:
            task_label = [list(range(classes_per_task * j, classes_per_task * (j + 1))) for j in range(args.tasks)]

    elif model.scenario == 'class':
        if model.experiment == 'expandMNIST':
            task_label = [list(range(j + classes_per_task)) for j in range(args.tasks)]
        elif model.experiment == 'splitMNIST':
            task_label = [list(range(classes_per_task * j)) for j in range(args.tasks)]
        elif model.experiment == 'singleMNIST':
            task_label = [list(range(j)) for j in range(args.tasks)]

    # Evaluate precision of final model on full test-set
    precs = [evaluate.validate(model, test_datasets[i], verbose=False, test_size=None, task=i + 1, allowed_classes=(task_label[args.task_order[i]] if args.change_order else task_label[i]) if model.scenario != 'domain' else None) for i in range(args.tasks)]
    ...
    if (utils.checkattr(args, 'feedback') or train_gen) and args.experiment=='CIFAR100' and args.scenario == 'class':
        ...
        if verbose:
            print('=> Log-likelihood on test set: {:.4f} +/- {:.4f}\n'.format(np.mean(ll_per_datapoint), np.sqrt(np.var(ll_per_datapoint))))
        # -write out to text file
        ...
        if verbose:
            print('=> Reconstruction error (per input unit) on test set: {:.4f} +/- {:.4f}\n'.format(np.mean(re_per_datapoint), np.sqrt(np.var(re_per_datapoint))))
        # -write out to text file
        ...
        try:
            utils.load_checkpoint(pretrained_classifier, args.m_dir, verbose=True, name='{}{}'.format(pretrained_classifier.name, eval_tag))
            FileFound = True
    ...
    if args.pdf or plot:
        ...
        if args.train and (not utils.checkattr(args, 'only_last')):
            ...
            figure = evaluate.visual.plt.plot_lines(precision_dict['all_tasks'], x_axes=precision_dict['x_task'], line_names=['{} {}'.format('episode' if args.scenario == 'class' else 'task', i + 1) for i in range(args.tasks)], xlabel='# of {}s'.format('episode' if args.scenario == 'class' else 'task'), ylabel='Test accuracy')
            figure_list.append(figure)
            figure = evaluate.visual.plt.plot_lines([precision_dict['average']], x_axes=precision_dict['x_task'], line_names=['Average based on all {}s so far'.format(('digit' if args.experiment == 'splitMNIST' else 'classe') if args.scenario else 'task')], xlabel='# of {}s'.format('episode' if args.scenario == 'class' else 'task'), ylabel='Test accuracy')
            figure_list.append(figure)
            # -add figures to pdf
        ...
        if gen_eval and not no_samples:
            evaluate.show_samples(model if utils.checkattr(args, 'feedback') else generator, config, size=args.sample_n, pdf=pp, title='Generated samples (by final model)')
