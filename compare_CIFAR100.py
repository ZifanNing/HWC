...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_CIFAR100', description='Compare performance of continual learning strategies on different scenarios of split CIFAR-100.')
...
if __name__ == '__main__':
    ...
    names = ['None', 'LwF', 'EWC', 'SI', 'Generative Replay (GR)', 'Brain-Inspired Replay (BI-R)', 'BI-R + SI'  if args.scenario == 'class' else 'Joint']
    ...
    figure = plt.plot_bar(means, names=names, colors=colors, ylabel='Test accuracy (after all 10 tasks)', title=title, yerr=sems if args.n_seeds > 1 else None, ylim=(0,1))
    ...
    figure = plt.plot_lines(ave_lines, x_axes=[2 * i for i in x_axes] if args.scenario == 'class' else x_axes, line_names=names, colors=colors, title=title, xlabel='# {} so far'.format('classes' if args.scenario == 'class' else 'tasks'), ylabel='Test accuracy (on tasks seen so far)', list_with_errors=sem_lines if args.n_seeds > 1 else None, ylim=ylim)
    figure_list.append(figure)
