...
def handle_inputs():
    ...
    parser = options.define_args(filename='_compare_permMNIST100_hyperParams', description='Compare hyperparameters for permMNIST.')
...
if __name__ == '__main__':
    ...
    # -collect data
    ave_prec_bir = [BIR[dg_prop][0.] for dg_prop in gating_list]
    # -print on screen
    print('\n\nBRAIN-INSPIRED REPLAY (BI-R)')
    print(' param-list (dg_prop): {}'.format(gating_list))
    print('  {}'.format(ave_prec_bir))
    print('--->  dg_prop = {}     --    {}'.format(gating_list[np.argmax(ave_prec_bir)], np.max(ave_prec_bir)))
    ...
    ave_prec_bir_per_c = []
    for dg_prop in gating_list:
        ave_prec_bir_per_c.append([BIR[dg_prop][c] for c in ext_c_list])
    # -print on screen
    print('\n\nBI-R & SI')
    print(' param-list (si_c): {}'.format(ext_c_list))
    curr_max = 0
    for dg_prop in gating_list:
        ave_prec_temp = [BIR[dg_prop][c] for c in ext_c_list]
    ...
    if args.per_bir_comp:
        ...
        # -collect data
        ave_prec_bir_no_rtf = [BIR_no_RTF[dg_prop] for dg_prop in gating_list]
        ave_prec_bir_no_con = [BIR_no_CON[dg_prop] for dg_prop in gating_list]
        ave_prec_bir_no_dis = [BIR_no_DIS[dg_prop] for dg_prop in gating_list]
        ave_prec_gr_plus_gat = [GR_plus_GAT[dg_prop] for dg_prop in gating_list]
        # -print on screen
        print('\n\nBI-R minus REPLAY-THROUGH-FEEDBACK')
        print(' param-list (dg_prop): {}'.format(gating_list))
        print('  {}'.format(ave_prec_bir_no_rtf))
        print('--->  dg_prop = {}     --    {}'.format(gating_list[np.argmax(ave_prec_bir_no_rtf)], np.max(ave_prec_bir_no_rtf)))
        print('\n\nBI-R minus CONDITIONAL REPLAY')
        print(' param-list (dg_prop): {}'.format(gating_list))
        print('  {}'.format(ave_prec_bir_no_con))
        print('--->  dg_prop = {}     --    {}'.format(gating_list[np.argmax(ave_prec_bir_no_con)], np.max(ave_prec_bir_no_con)))
        print('\n\nBI-R minus DISTILLATION')
        print(' param-list (dg_prop): {}'.format(gating_list))
        print('  {}'.format(ave_prec_bir_no_dis))
        print('--->  dg_prop = {}     --    {}'.format(gating_list[np.argmax(ave_prec_bir_no_dis)], np.max(ave_prec_bir_no_dis)))
        print('\n\nGR plus GATING BASED ON INTERNAL CONTEXT')
        print(' param-list (dg_prop): {}'.format(gating_list))
        print('  {}'.format(ave_prec_gr_plus_gat))
        print('--->  dg_prop = {}     --    {}'.format(gating_list[np.argmax(ave_prec_gr_plus_gat)], np.max(ave_prec_gr_plus_gat)))
    print('\n')
    ...
    full_list = [item for sublist in ave_prec_per_lambda for item in sublist] + ave_prec_si + [item for sublist in ave_prec_bir_per_c for item in sublist]
    if args.per_bir_comp:
        full_list += (ave_prec_bir_no_rtf + ave_prec_bir_no_con + ave_prec_bir_no_dis + ave_prec_gr_plus_gat)
    ...
    if len(lamda_list) > 0:
        ...
        figure = my_plt.plot_lines(ave_prec_per_lambda, x_axes=[0] + lamda_list, ylabel=ylabel, line_names=['EWC'] + ['Online EWC - gamma = {}'.format(gamma) for gamma in gamma_list], title=title, x_log=True, xlabel='EWC: lambda (log-scale)', ylim=(miny - marginy, maxy + marginy), with_dots=True, colors=colors, h_line=BASE, h_label='None')
        figure_list.append(figure)

    ###---SI---###
    figure = my_plt.plot_lines([ave_prec_si], x_axes=[0] + c_list, ylabel=ylabel, line_names=['SI'], colors=['yellowgreen'], title=title, x_log=True, xlabel='SI: c (log-scale)', with_dots=True, ylim=(miny - marginy, maxy + marginy), h_line=BASE, h_label='None')
    figure_list.append(figure)

    ###---Brain-Inspired Replay---###
    figure = my_plt.plot_lines([ave_prec_bir], x_axes=gating_list, ylabel=ylabel, line_names=['Brain-Inspired Replay (BI-R)'], colors=['purple'], title=title, x_log=False, xlabel='Context gates: % of nodes gated', with_dots=True, ylim=(miny - marginy, maxy + marginy), h_lines=[BASE], h_labels=['None'], h_colors=['grey'])
    figure_list.append(figure)
    ...
    # - make plot (line plot - only average)
    figure = my_plt.plot_lines(ave_prec_bir_per_c, x_axes=[0] + c_list, ylabel=ylabel, line_names=['BI-R, gate-prop = {}'.format(dg_prop) for dg_prop in gating_list], title=title, x_log=True, xlabel='BI-R + SI: c (log-scale)', ylim=(miny - marginy, maxy + marginy), with_dots=True, colors=colors, h_line=BASE, h_label='None')
    figure_list.append(figure)

    ###---BI-R per component---###
    if args.per_bir_comp:
        figure = my_plt.plot_lines([ave_prec_bir_no_rtf, ave_prec_bir_no_con, ave_prec_bir_no_dis, ave_prec_gr_plus_gat], x_axes=gating_list, ylabel=ylabel, line_names=['BI-R - rtf', 'BI-R - con', 'BI-R - dis', 'GR + gat'], colors=['maroon', 'red', 'green', 'darkorange'], title=title, x_log=False, xlabel='Context gates: % of nodes gated', with_dots=True, ylim=(miny - marginy, maxy + marginy), h_lines=[BASE], h_labels=['None'], h_colors=['grey'])
        figure_list.append(figure)
